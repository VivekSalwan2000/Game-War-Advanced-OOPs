...
public class Protocol {
    ...
    public void processInput(RequestType requestType) throws InvalidProtocolRequestException, IOException {
        switch (requestType) {
            ...
            case CREATE_VILLAGE_REQUEST -> {
                ...
                awaitTask(executor.submit(villageControllerGenerator));
            ...
            case CREATE_BUILDING_REQUEST -> {
                ...
                awaitTask(executor.submit(buildingFactory));
            ...
            case CREATE_WORKER_REQUEST -> {
                ...
                awaitTask(executor.submit(workerFactory));
    ...
    /**
     * Blocks until the submitted task completes, without busy-waiting on {@code isDone()}.
     * Replaces the previous {@code while(!future.isDone()){}} spin loops.
     * @param future the submitted task to await
     */
    private void awaitTask(Future<?> future) {
        try {
            future.get();
    ...
    /**
     * Releases this protocol's executor. Should be called when the client session ends
     * so that per-connection thread pools are not leaked.
     */
